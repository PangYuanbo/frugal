...
const PROVIDERS = [
  { name: 'Vercel', pattern: cmd('vercel|vc'),
    hint: 'Hobby (non-commercial only): 1M invocations + 1M edge reqs, 360 GB-hr memory, 100 GB transfer, 100 deploys/day — blown quotas hard-pause ~30 days. Pro fails OPEN: Spend Management is a $200 NOTIFICATION by default, not a cap. #1 trap: LLM streaming/polling bills Provisioned Memory during I/O waits. Check: `vercel usage`' },
  { name: 'Cloudflare', pattern: cmd('wrangler|cloudflared'),
    hint: 'free tier is per-DAY and hard-fails: 100k Worker reqs + 10ms CPU; KV 1,000 writes/day blows first; D1 100k writes/day. Paid $5/mo = 10M reqs but fails OPEN past that (KV writes $5/M, DO duration $12.50/M GB-s, no hard cap). Prefer `wrangler dev` locally; spend via dashboard Billable Usage' },
  ...
  { name: 'Neon', pattern: cmd('neonctl|neon'),
    hint: 'free: 100 CU-hr/mo per project, 0.5 GB storage, 5 GB egress — overrun SUSPENDS the DB until next month. Paid (Launch/Scale) fails OPEN: same polling pattern becomes a silent bill (~$77/mo per always-on CU), egress past 500 GB bills $0.10/GB — SET a spend limit. Use a branch for tests' },
  ...
  { name: 'GitHub Actions', pattern: cmd('gh\\s+(?:workflow|run)'),
    hint: 'free: 2,000 private-repo minutes/mo (public repos unlimited); an auto-deploy pipeline eats that in 2-3 weeks, then workflows SILENTLY fail at the default $0 spending limit — but a raised limit fails OPEN (unbounded per-minute billing, macOS ~10x Linux). Check: `gh api /users/{user}/settings/billing/usage`' },
  ...
  { name: 'GCP', pattern: cmd('gcloud|gsutil|bq|firebase\\s+deploy'),
    hint: 'Cloud Run free: 2M req + 180K vCPU-s/mo BUT only 1 GB egress; min-instances bills 24/7 — avoid it. No default cap: set alerts via `gcloud billing budgets`; spend via billing console' },
