...
```mermaid
flowchart LR
    A["agent 执行<br/>vercel deploy"] --> B{frugal<br/>PreToolUse hook}
    B -->|"本会话<br/>首次触碰"| C["注入一行提醒:<br/>免费额度墙 · 头号陷阱 ·<br/>消费上限状态 · 查用量命令"]
    B -->|"已提醒过 /<br/>今日已达 5 次上限"| D[静默]
    C --> E["agent 核实 plan 与用量,<br/>一句话告知用户,<br/>再谨慎地继续"]
```

两层机制，上下文成本都很低（每会话固定约 2KB 规则 + 每天最多 5 条一行提醒）：

```mermaid
flowchart TB
    subgraph always["SessionStart — 规则（常驻，约 2KB）"]
        R1["首次碰服务先查用量 · 创建付费资源先报备 ·<br/>临时资源必须死 · 告警不是刹车 ·<br/>付费档 fail OPEN · 爬虫也算你的账"]
    end
    subgraph jit["PreToolUse — 数据（按需注入，按服务）"]
        K1["vercel → Vercel 的数字"]
        K2["wrangler r2 → R2 的数字"]
        K3["git push + workflows/ → Actions 分钟数"]
        K4["命令里出现 sk-… → 密钥泄漏警告"]
    end
    always -.->|"该怎么做"| Agent
    jit -.->|"相关时给出精确数字"| Agent
...
frugal 内化的 272 条 X 真实事故里几个代表案例，以及现在会先拦住它们的检测点：

| 真实账单 | 发生了什么 | frugal 的拦截 |
|---|---|---|
| **$600,000** | 泄漏的 OpenAI key 被盗用 | 命令/`.env` 里出现密钥字面量 → 警告 + 建议轮换 |
